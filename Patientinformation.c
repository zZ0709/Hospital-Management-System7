...
void creat_patientlist() {//需传入链表头， #默认原文件数据正确,数据个数不超范围,存历史数据非当日#
	int readcheck;//检验是否读取正确
	ppatient p, patientrear = ppatienthead;
	...
	while (patientrear->next_patient != NULL)  patientrear = patientrear->next_patient;//  先找到 head 链表目前的末尾，确保是从末尾开始接数据
...
void initial_today_patient(patientlist p,Date day) {//########需要doctornumber初始化为0
...
ppatient search_patientlist(char id[19]) { 
	ppatient p;//p为p0后驱
...
void add_patientlist(patientlist list) {
